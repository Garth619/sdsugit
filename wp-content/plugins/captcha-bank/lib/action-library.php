	...
	else
	{
		...
		if(isset($_REQUEST["param"]))
		{
			...
			switch(esc_attr($_REQUEST["param"]))
			{
				...
				case "captcha_alert_setup_module":
					if(wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "captcha_bank_alert_setup"))
					{
						parse_str(isset($_REQUEST["data"]) ? base64_decode($_REQUEST["data"]) : "",$alert_setup_data);
						$update_captcha_type = array();
						$where = array();

						$update_captcha_type["email_when_a_user_fails_login"] = esc_attr($alert_setup_data["ux_ddl_fail"]);
						$update_captcha_type["email_when_a_user_success_login"] = esc_attr($alert_setup_data["ux_ddl_success"]);
						$update_captcha_type["email_when_an_ip_address_is_blocked"] = esc_attr($alert_setup_data["ux_ddl_Ip_address"]);
						$update_captcha_type["email_when_an_ip_address_is_unblocked"]= esc_attr($alert_setup_data["ux_ddl_address"]);
						$update_captcha_type["email_when_an_ip_range_is_blocked"] = esc_attr($alert_setup_data["ux_ddl_Ip"]);
						$update_captcha_type["email_when_an_ip_range_is_unblocked"] = esc_attr($alert_setup_data["ux_ddl_range"]);
				...
				case "captcha_manage_ip_address_module":
					if(wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "captcha_manage_ip_address"))
					{
						...
						$ip = isset($_REQUEST["ip_address"]) ? doubleval($_REQUEST["ip_address"]) : 0;
						...
						else
						{
							...
							$insert_manage_ip_address["date_time"] = time();
				...
				case "captcha_manage_ip_ranges_module":
					if(wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? esc_attr($_REQUEST["_wp_nonce"]) : "", "captcha_manage_ip_ranges"))
					{
						...
						$start_ip_range = isset($_REQUEST["start_range"]) ? doubleval($_REQUEST["start_range"]) : 0;
						$end_ip_range = isset($_REQUEST["end_range"]) ? doubleval($_REQUEST["end_range"]) : 0;
						$blocked_for = esc_attr($ip_range_data["ux_ddl_blocked"]);
						$get_ip = get_ip_location_captcha_bank(long2ip($start_ip_range.",".$end_ip_range));
						$location = $get_ip->country_name == "" && $get_ip->city == "" ? "" : $get_ip->country_name == ""  ?  "" : $get_ip->city == "" ? $get_ip->country_name : $get_ip->city .", ".$get_ip->country_name;
