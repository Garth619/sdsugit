...
else
{
	...
	else
	{
		...
		*/
				...
				foreach($parent_table_data as $row)
				{
					switch($row->type)
					{
						...
						case "other_settings":
							...
							$other_settings_data["other_settings_error_reporting"] = "enable";
		...
		switch($captcha_version_no)
		{
			...
			default:
				...
				else
				{
					...
					if(!isset($other_settings_unserialize_data["other_settings_error_reporting"]))
					{
						$other_settings_unserialize_data["other_settings_error_reporting"] = "enable";
						$where = array();
		...
		update_option("captcha-bank-version-number","4.0.0");
	}
}
?>
